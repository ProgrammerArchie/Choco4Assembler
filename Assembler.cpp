...
int main(void){
  ...
  for(unsigned i = 0; i < tokens.size(); ++i){
    ...
    if(tokens[i] == "STL") byte = 0b00000000;
      ...
      else byte += ((uint8_t) std::strtoul(tokens[i]) * 4);
      if(std::strtoul(tokens[++i]) > 3){
        byte += 0b11;
      ...
      byte = 0b11010000;
      if(((std::strtoul(tokens[i + 1]) & 0x0f00) >> 8) > 0b1111){
        byte += 0b1111;
        std::cout << "WARNING: 'JMP [addr top nibble]' <- [addr top nibble] > 15, [addr top nibble] ROUNDED DOWN TO 15.\n";
      }
      else byte += (uint8_t) ((std::strtoul(tokens[i + 1]) & 0x0f00) >> 8);
      bytes = 2;
    }
    ...
    if(bytes > 1){
      ...
      for(uint8_t i = 0; i < bytes; ++i){
        byte = 0xff & (uint8_t) std::strtoul(tokens[++i]);
        ofile << byte;
      }
    }
  }
